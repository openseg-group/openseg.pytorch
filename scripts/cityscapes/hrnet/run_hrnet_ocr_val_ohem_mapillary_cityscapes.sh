...
BACKBONE="hrnet48"
CONFIGS="configs/cityscapes/${BACKBONE}_mapillary.json"
...
CHECKPOINTS_NAME="${MODEL_NAME}_${BACKBONE}_${BN_TYPE}_${BATCH_SIZE}_${MAX_ITERS}_val_ohem_mapillary_"$2
...
if [ "$1"x == "train"x ]; then
  ...
  ${PYTHON} -u main.py --configs ${CONFIGS} --drop_last y --train_batch_size ${BATCH_SIZE} --only_mapillary y --base_lr 0.0001 \
                       --phase train --gathered n --loss_balance y --log_to_file n \
