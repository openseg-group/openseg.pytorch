...
CONFIGS_CROP_TEST="configs/cityscapes/${BACKBONE}_crop_test.json"
...
CHECKPOINTS_NAME="${MODEL_NAME}_${BACKBONE}_${BN_TYPE}_${BATCH_SIZE}_${MAX_ITERS}_val_ohem_fine_"$2
LOG_FILE="./log/cityscapes/${CHECKPOINTS_NAME}.log"
echo "Logging to $LOG_FILE"
mkdir -p `dirname $LOG_FILE`

PRETRAINED_MODEL="./checkpoints/cityscapes/hrnet48_ocr_hrnet48__8_100000_val_ohem_1_max_performance.pth"
# hrnet48_ocr_hrnet48__8_100000_val_ohem_1_max_performance.pth
...
if [ "$1"x == "train"x ]; then
  ...
  ${PYTHON} -u main.py --configs ${CONFIGS} --drop_last y --train_batch_size ${BATCH_SIZE} --include_val y --base_lr 0.0001 \
                       --phase train --gathered n --loss_balance y --log_to_file n \
  ...
  elif [ "$3"x == "mscrop"x ]; then
    echo "[mscrop + multiple scale + flip] test"
    ${PYTHON} -u main.py --configs ${CONFIGS_CROP_TEST} --drop_last y \
                         --backbone ${BACKBONE} --model_name ${MODEL_NAME} --checkpoints_name ${CHECKPOINTS_NAME} \
                         --phase test --gpu 0 1 2 3 --resume ./checkpoints/cityscapes/${CHECKPOINTS_NAME}_latest.pth \
                         --test_dir ${DATA_DIR}/test --log_to_file n \
                         --out_dir ${SAVE_DIR}${CHECKPOINTS_NAME}_test_ms_crop
  else
    echo "[while image + multiple scale + flip] test"
