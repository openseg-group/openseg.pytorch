...
class ModuleRunner(object):
    ...
    def load_net(self, net):
        ...
        if self.configer.get("network", "resume") is not None:
            ...
            if hasattr(net, "module"):
                ...
                self.load_state_dict(
                    net, checkpoint_dict, self.configer.get("network", "resume_strict")
                )
        ...
        if save_mode == "performance":
            if self.configer.get("performance") > self.configer.get("max_performance"):
                ...
                self.configer.update(
                    ["max_performance"], self.configer.get("performance")
                )
    ...
    def warm_lr(self, iters, scheduler, optimizer, backbone_list=(0,)):
        ...
        """
        if not self.configer.exists("lr", "is_warm") or not self.configer.get(
            "lr", "is_warm"
        ):
            return
        ...
        if iters < warm_iters:
            if self.configer.get("lr", "warm")["freeze_backbone"]:
                ...
                for backbone_index in backbone_list:
                    optimizer.param_groups[backbone_index]["lr"] = base_lr_list[
                        backbone_index
                    ] * (lr_ratio ** 4)
