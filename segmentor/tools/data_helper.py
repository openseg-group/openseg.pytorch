...
class DataHelper:
    ...
    def _prepare_sequence(self, seq, force_list=False):
        def split_and_cuda(
            lst: "List[List[Tensor, len=N]]", device_ids
        ) -> "List[List[Tensor], len=N]":
            results = []
            ...
            sequences.extend(
                [
                    self._prepare_sequence(
                        [rev_data_dict[k] for k in input_keys], force_list=True
                    ),
                    self._prepare_sequence(
                        [rev_data_dict[k] for k in target_keys], force_list=False
                    ),
                ]
            )
