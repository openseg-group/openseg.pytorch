...
class Tester(object):
    ...
    def _crop_predict(self, image, crop_size):
        ...
        with torch.no_grad():
            ...
            if self.configer.get('loss', 'loss_type') == "pyramid_auxce_loss":
                sum_results = results[1] + results[2] + results[3] + results[4]
                results = sum_results.permute(0, 2, 3, 1).cpu().numpy()
    ...
    def _predict(self, inputs):
        with torch.no_grad():
            ...
            if self.configer.get('loss', 'loss_type') == "pyramid_auxce_loss":
                sum_results = results[1] + results[2] + results[3] + results[4]
                results = sum_results.squeeze(0).permute(1, 2, 0).cpu().numpy()
