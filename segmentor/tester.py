...
from lib.vis.palette import (
    get_pascal_context_colors,
    get_cocostuff_colors,
    get_pascal_voc_colors,
)
...
class Tester(object):
    ...
    def test(self, data_loader=None):
        ...
        for j, data_dict in enumerate(self.test_loader):
            ...
            with torch.no_grad():
                # Forward pass.
                if (
                    self.configer.exists("data", "use_offset")
                    and self.configer.get("data", "use_offset") == "offline"
                ):
    ...
    def sscrop_test(self, inputs, crop_size, scale=1):
        ...
        n, c, h, w = (
            scaled_inputs.size(0),
            scaled_inputs.size(1),
            scaled_inputs.size(2),
            scaled_inputs.size(3),
        )
        full_probs = torch.cuda.FloatTensor(
            n, self.configer.get("data", "num_classes"), h, w
        ).fill_(0)
        ...
        for height in height_starts:
            for width in width_starts:
                crop_inputs = scaled_inputs[
                    :, :, height : height + crop_size[0], width : width + crop_size[1]
                ]
        ...
        full_probs = F.interpolate(
            full_probs, size=(ori_h, ori_w), mode="bilinear", align_corners=True
        )
        return full_probs
    ...
    def ms_test(self, inputs):
        if isinstance(inputs, torch.Tensor):
            ...
            full_probs = [
                torch.zeros(
                    1, self.configer.get("data", "num_classes"), i.size(1), i.size(2)
                ).cuda(device_ids[index], non_blocking=True)
            ...
            if self.configer.exists("test", "scale_weights"):
                for scale, weight in zip(
                    ...
                    for i in range(len(inputs)):
                        full_probs[i] += weight * (
                            probs[i] + self.flip(flip_probs[i], 3)
                        )
                return full_probs
            else:
                for scale in self.configer.get("test", "scale_search"):
    ...
    def fuse_with_depth(self, probs, scales, names):
        ...
        for index, name in enumerate(names):
            ...
            for prob, scale in zip(probs, scales):
                ...
                full_probs[index, :, :, :] += (
                    torch.from_numpy(np.expand_dims(weight_map, axis=0)).type(
    ...
    def dense_crf_process(self, images, outputs):
        ...
        b = images.size(0)
        mean_vector = np.expand_dims(
            np.expand_dims(
                np.transpose(np.array([102.9801, 115.9465, 122.7717])), axis=1
            ),
            axis=2,
        )
