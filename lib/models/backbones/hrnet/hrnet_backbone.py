...
class HighResolutionNext(nn.Module):
    ...
    def __init__(self, cfg, bn_type, bn_momentum, **kwargs):
        ...
        num_channels = [
            num_channels[i] * block.expansion for i in range(len(num_channels))]
        self.transition0 = self._make_transition_layer([64], num_channels, bn_type=bn_type, bn_momentum=bn_momentum)
        self.stage1, pre_stage_channels = self._make_stage(
            self.stage1_cfg, num_channels, bn_type=bn_type, bn_momentum=bn_momentum)
...
class HRNetBackbone(object):
    ...
    def __call__(self):
        ...
        if arch == 'hrnet18':
            ...
            arch_net = HighResolutionNet(MODEL_CONFIGS['frnet8'], 
                                         bn_type='inplace_abn',
                                         bn_momentum=0.1)
            arch_net = ModuleHelper.load_model(arch_net, 
                                               pretrained=self.configer.get('network', 'pretrained'), 
            ...
            arch_net = HighResolutionNet(MODEL_CONFIGS['frnet16'], 
                                         bn_type='inplace_abn',
                                         bn_momentum=0.1)
            arch_net = ModuleHelper.load_model(arch_net, 
                                               pretrained=self.configer.get('network', 'pretrained'), 
            ...
            arch_net = HighResolutionNet(MODEL_CONFIGS['frnet24'], 
                                         bn_type='inplace_abn',
                                         bn_momentum=0.1)
            arch_net = ModuleHelper.load_model(arch_net, 
                                               pretrained=self.configer.get('network', 'pretrained'), 
            ...
            arch_net = HighResolutionNet(MODEL_CONFIGS['frnet36'], 
                                         bn_type='inplace_abn',
                                         bn_momentum=0.1)
            arch_net = ModuleHelper.load_model(arch_net, 
                                               pretrained=self.configer.get('network', 'pretrained'), 
