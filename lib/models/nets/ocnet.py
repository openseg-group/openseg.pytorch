...
class BaseNOCNet(nn.Module):
    def __init__(self, configer):
        self.inplanes = 128
        super(BaseNOCNet, self).__init__()
        self.configer = configer
        self.num_classes = self.configer.get('data', 'num_classes')
        self.backbone = BackboneSelector(configer).get_backbone()
        ...
        if "wide_resnet38" in self.configer.get('network', 'backbone'):
            ...
            in_channels = [1024, 2048]
        self.noc_module_pre = nn.Sequential(
            nn.Conv2d(in_channels[1], 512, kernel_size=3, stride=1, padding=1),
            ModuleHelper.BNReLU(512, bn_type=self.configer.get('network', 'bn_type')),
        )
        from lib.models.modules.base_noc_block import BaseNOC_Module
        self.noc_module = BaseNOC_Module(in_channels=512, out_channels=512,
...
class PyramidOCNet(nn.Module):
    ...
    def forward(self, x_):
        ...
        feats = self.context(feats, x[-1])
        x = self.cls_head(feats)
        x_dsn = F.interpolate(x_dsn, size=(x_.size(2), x_.size(3)), mode="bilinear", align_corners=True)
        x = F.interpolate(x, size=(x_.size(2), x_.size(3)), mode="bilinear", align_corners=True)
        return x_dsn, x
